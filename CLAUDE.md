...
这是一个仅限 Windows 的 WPF 应用程序（`net8.0-windows`），无法在 Linux/macOS 上构建或运行。

## 架构概览

**基于 YOLO 的火灾检测系统** — 使用 MVVM 模式、Prism 框架和 DryIoc 依赖注入的 WPF 桌面应用程序。

### 层次结构

- **`Core/`** — `FireDetectionModule`（静态单例）：加载并封装 YoloSharp 预测器。所有 ViewModel 通过它进行检测。三个入口点：`DetectAsync(imagePath)` 用于图片，`Detect(Image)` 用于 ImageSharp 对象，`DetectFrame(Mat)` 用于 OpenCV 帧。

- **`Services/`** — 通过 Prism/DryIoc 注入到 ViewModel 的业务逻辑层：
...
- **`ViewModels/`** — 每个 View 都有配对的 ViewModel。导航由 Prism 的 `RegionManager` 处理；名为 `"MainRegion"` 的区域（在 `MainView.xaml` 中）承载活动页面。
...
3. `OnInitialized()`：
   - 初始化 SQLite 数据库（自动创建架构 + 默认 `admin`/`admin123` 账户）
   - 从 `appsettings.json` 的 `ModelSettings:ModelPath` 加载 YOLO 模型
...
| 模式 | ViewModel | 关键行为 |
|------|-----------|---------|
| 图片 | `ImageDetectionViewModel` | 异步检测；保存 `DetectionRecord` 到数据库；如果检测到火灾且超过阈值则触发报警 |
| 视频 | `VideoDetectionViewModel` | 逐帧同步检测；120ms UI 节流；支持暂停/继续 |
| 摄像头 | `CameraDetectionViewModel` | 多源并发检测；使用视频文件模拟摄像头流；循环播放 |

### 配置文件（`appsettings.json`）

`ModelSettings` 中的 `ModelPath` **必须指向有效的 ONNX 文件**，应用才能运行检测。当前值是开发者特定的绝对路径 — 请更新它或使用相对路径如 `models/best.onnx`。

主要配置部分：
- `ModelSettings`：模型路径、置信度阈值（默认 0.5）、NMS 阈值、输入尺寸
- `AlarmSettings`：启用/禁用声音、桌面通知、邮件；`AlertThreshold`（默认 0.7）
- `DatabaseSettings`：SQLite 连接字符串（默认：`Data Source=firedetection.db`）
- `EmailSettings`：邮件报警的 SMTP 配置
- `Logging`：日志级别和滚动文件路径

### 关键实现要点

- `FireDetectionModule` 使用双重检查锁定实现线程安全的单例初始化
- 视频/摄像头检测将 OpenCV `Mat`（BGR）→ JPEG 字节流 → ImageSharp `Image`（RGB）后再传递给 YOLO
- `CameraDetectionViewModel` 使用循环播放的视频文件模拟摄像头
- 所有密码使用 BCrypt 哈希；默认账户为 `admin`/`admin123`
- 数据库在启动时通过 EF Core 的 `EnsureCreated()` 自动创建
